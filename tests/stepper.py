...
def step_absolute(the_board):
    ...
    speed = 2500
    the_board.stepper_set_max_speed(motor, speed)
    the_board.stepper_set_speed(motor, speed)
    the_board.stepper_set_acceleration(motor, 1000)

    # set the absolute position in steps
    the_board.stepper_move_to(motor, -3000)

    # run the motor
    print('Starting motor... 1/2')
    the_board.stepper_run(motor, completion_callback=the_callback)
    time.sleep(.2)
    the_board.stepper_is_running(motor, callback=running_callback)
    time.sleep(10)
    print('Starting motor... 2/2')
    the_board.stepper_move_to(motor, 0)
    the_board.stepper_run(motor, completion_callback=the_callback)
    time.sleep(.2)
    the_board.stepper_is_running(motor, callback=running_callback)
    time.sleep(10)
